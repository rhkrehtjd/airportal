...
ap_str <- "인천: RKSI, 김포: RKSS, 청주: RKTU, 양양: RKNY, 군산: RKJK, 원주: RKNW, 김해: RKPK, 제주: RKPC, 대구: RKTN, 광주: RKJJ, 여수: RKJY, 울산: RKPU, 포항: RKTH,  사천: RKPS, 무안: RKJB"
ap_str <- gsub(" ", "", ap_str)
...
for (dep_arr in c("D", "A")) {
  for (date in as.character(20070102:20070103)) {
    
    result_ <- data.frame()
    
    for (airport in c("인천", "김포", "청주", "양양", "군산", "원주", "김해",
                      "제주", "대구", "광주", "여수", "울산", "포항", "사천", "무안")) {
      data <- get_airplane(dep_arr, date, airport)
      if(length(data)==10){result_ <- rbind(result_, data)}else{}
    }
    
    if (dep_arr == "A") {colnames(result_)[1:8] <- c("항공사", "편명", "출발지", "계획", "예상", "도착", "구분", "현황")
    } else colnames(result_)[1:8] <- c("항공사", "편명", "목적지", "계획", "예상", "출발", "구분", "현황")
    
    
    csv_file_name <- paste0(date, "_", dep_arr, ".csv")
    write.csv(result_, file = csv_file_name, row.names = FALSE)
    drive_upload(csv_file_name, name = csv_file_name)
